...
// Mirrors AUTO_PUBLISH_CONFIDENCE_LEVELS in
// apps/api/src/modules/news-intelligence/autonomous-publishing.service.ts -
// the backend re-validates against this same fixed list regardless of what
// this form sends, so it's the source of truth; kept in sync here manually.
const AUTONOMOUS_AUTO_PUBLISH_THRESHOLD_KEY = "news.autonomous_pipeline.auto_publish_confidence_threshold";
const AUTO_PUBLISH_LEVELS = [70, 80, 90, 100] as const;
...
function PublishQuotaFields({
  ...
  // Seeded once at mount from already-resolved settings data - plain
  // inputs, no later async update to resync from.
...
function AutonomousPublishingCard({ aiConfigured }: { aiConfigured: boolean }) {
  ...
  const { data: autoPublishThresholdSaved, isLoading: autoPublishThresholdLoading } = useSetting<number>(
    AUTONOMOUS_AUTO_PUBLISH_THRESHOLD_KEY,
  );
  ...
  const updateAutoPublishThreshold = useUpdateSetting(AUTONOMOUS_AUTO_PUBLISH_THRESHOLD_KEY);
  ...
  // AUTO_PUBLISH_LEVELS.includes(x as any) narrows: only a real 70/80/90/100
  // counts as "on" - anything else stored under this key (cleared, or never
  // set) means off, matching the backend's own re-validation.
  const autoPublishThreshold = AUTO_PUBLISH_LEVELS.includes(autoPublishThresholdSaved as any)
    ? (autoPublishThresholdSaved as (typeof AUTO_PUBLISH_LEVELS)[number])
    : null;
  ...
  const handleAutoPublishThresholdChange = async (level: (typeof AUTO_PUBLISH_LEVELS)[number], checked: boolean) => {
    // Checklist behaves as a single-select: checking one level clears any
    // other; unchecking the active level turns auto-publish off entirely
    // (back to always-review) rather than leaving a stale value behind.
    const next = checked ? level : null;
    try {
      await updateAutoPublishThreshold.mutateAsync({ value: next });
      toast.success(
        next != null
          ? `Auto-publish enabled at ${next}% confidence — matching drafts skip review and publish immediately`
          : "Auto-publish disabled — every draft goes back to always requiring human review",
      );
    } catch (err) {
      toast.error(err instanceof ApiError ? err.message : "Failed to save");
    }
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>
          ...
          your brand voice. By default every draft lands in Review for a
          human to check before it goes live — an automatic fact-check and
          quality gate still runs on each draft either way, so reviewers can
          see at a glance which ones passed. Optionally set a confidence
          threshold below to let high-confidence drafts publish immediately
      ...
      <CardContent className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-2 border-t pt-4">
          <Label>Auto-publish confidence threshold</Label>
          <p className="text-xs text-muted-foreground">
            When a draft passes the automated fact-check and quality gate
            AND reaches this confidence level, publish it immediately
            instead of sending it to Review. Leave every box unchecked to
            keep the current behavior — always send to Review regardless of
            confidence. A failed gate never auto-publishes, no matter how
            high the confidence number is.
          </p>
          <div className="flex flex-wrap gap-4">
            {AUTO_PUBLISH_LEVELS.map((level) => (
              <div key={level} className="flex items-center gap-2">
                <Checkbox
                  id={`auto-publish-threshold-${level}`}
                  checked={autoPublishThreshold === level}
                  onCheckedChange={(v) => handleAutoPublishThresholdChange(level, !!v)}
                />
                <Label htmlFor={`auto-publish-threshold-${level}`} className="font-normal">
                  {level}%
                </Label>
              </div>
            ))}
          ...
          {autoPublishThreshold != null && (
            <p className="text-xs font-medium text-primary">
              Auto-publish is ON at {autoPublishThreshold}% confidence.
            </p>
          )}
        </div>
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Every AI-related configuration for this deployment, in one place — visible only to superadmins.
        </p>
      </div>
