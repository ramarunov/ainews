...
export class SeriesService {
  ...
  private async generateSlug(
    ...
    while (true) {
      ...
      // so a slug "freed up" by a deleted series is still taken as far as
      // Postgres is concerned. Checking only active series let this pick a
      // slug that then failed with an unhandled P2002 at create().
